...
<Callout>Requires the `Message Content` intent - information is read from CFTools webhoook with `user.join` event</Callout>

<Callout type="info">Check out [our guide](/docs/gameshield-getting-started) if you need help with this part - it's made for a different project, but the process is the same</Callout>
...
| `CFTOOLS_WEBHOOK_CHANNEL_ID` | The id of the channel where CFTools webhook (`user.join`) messages are send  | string |

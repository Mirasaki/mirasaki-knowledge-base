...
Please read the Terms of Service carefully/thoroughly. By using the **GameShield** bot you agree to these terms of service.

The **GameShield** [Discord](https://discord.com) bot, further referred to as "the service", "the bot" or "GameShield", is meant to enhance the DayZ player experience by providing kill-feeds (and other event-feeds) in your Discord server. To ensure that we can deliver the best possible experience, and to prevent malicious actors from abusing the services **GameShield** offers, we've established the following terms of service:

## Rules of Conduct

When using **GameShield**, you must follow the rules (as presented below this paragraph) in order to keep both our users (you) and the services **GameShield** uses (currently only [The Discord API](https://discord.dev)) safe. In order to achieve this, we (**GameShield** and it's developers) reserve the right to prevent users from further using the entire service, or parts of the service.
